...
clustering.function <- function(data, W){
  ...
  for (i in 1:(nrow(data)-1)){
    ...
    # }
    cluster.store.factor <- factor(cluster.store[i,], levels=seq(1,nrow(data)))
    cluster.size <- as.numeric(table(cluster.store.factor))
  ...
  # }
  #W.list <- replicate(nrow(data), matrix(data=NA,nrow=nrow(data),ncol=nrow(data),byrow=FALSE,dimnames=NULL), simplify=FALSE)
